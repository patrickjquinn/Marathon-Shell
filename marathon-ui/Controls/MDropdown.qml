...
Item {
    ...
    Accessible.onPressAction: if (!disabled) toggle()
    
    Keys.onSpacePressed: if (!disabled) toggle()
    Keys.onReturnPressed: if (!disabled) toggle()
    Keys.onUpPressed: if (expanded) selectPrevious()
    Keys.onDownPressed: if (expanded) selectNext()
    Keys.onEscapePressed: if (expanded) collapse()
    ...
    Column {
        ...
        Rectangle {
            ...
            radius: MRadius.md
            color: {
                if (root.disabled) return MColors.bb10Surface
                if (mouseArea.pressed) return MColors.highlightSubtle
    ...
    Rectangle {
        ...
        ListView {
            ...
            delegate: Rectangle {
                ...
                height: 44
                color: {
                    if (index === root.selectedIndex) return MColors.highlightMedium
