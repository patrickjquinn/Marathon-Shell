...
Rules for writing C++, QML, and shell code in this repo. The goal is a codebase that is dense and idiomatic: no defensive bloat, no resurrection of patterns the toolkit already provides.
...
These produce slow, fragile, or misleading code. They show up disproportionately in generated code and in tired humans. Catch them in review.
...
Before committing, scan for these. They show up regardless of language.
...
## Section F — Process rules for generated or unfamiliar code

- **Always run** the change in the actual binary before claiming it works. "It compiles" is not "it works".
- **Verify imports/symbols exist** whenever a name is unfamiliar. Invented APIs -- functions, classes or headers that look right but aren't -- are the single most common defect in generated code.
- **Diff-review every line** of a generated or bulk-applied change the same way you'd review a stranger's PR. The author has to internalise it -- otherwise the reviewer is doing all the work.
...
- **No commit-message theatre.** Don't add generated-by or co-author trailers. Don't write commit messages that sound like a Medium article.

---

## Section G — Atomic, considered commits

Every commit is a discrete unit of meaning. Bulk or scripted edits make it easy to dump a bundle of half-related changes; that habit is forbidden here.
...
`build/`, `*.o`, `.qmlc`, `.cache/`, `core.*`, `*.swp`, `*.bak`. Add to `.gitignore` once; never again.

### G7. Don't commit tool trailers, "Generated by …" markers, or machine co-author lines

Per **Section A** + policy in this repo. Commit messages are human attribution: the person who lands the change is the one accountable for it.
...
- Keep the relevant section to hand when working on the corresponding code.
