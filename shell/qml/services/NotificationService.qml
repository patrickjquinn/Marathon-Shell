...
QtObject {
    ...
    function sendNotification(appId, title, body, options) {
        ...
        var notification = {
            ...
            "category": (options && options.category) ? options.category : "message",
            "priority": (options && options.priority) ? options.priority : "normal",
            "actions": (options && options.actions) ? options.actions : [],
            "persistent": (options && typeof options.persistent !== "undefined") ? options.persistent : false
        };
    ...
    function _platformNotify(notification) {
        if (Platform.isLinux) {
            ...
            if (typeof FreedesktopNotifications !== 'undefined') {
                ...
                var newId = FreedesktopNotifications.Notify(notification.appId, replacesId, notification.icon || "", notification.title, notification.body, actionsList, hints, notification.persistent ? 0 : 5000);
