...
QtObject {
    ...
    function getSignalIcon(strength) {
        ...
        if (strength === 0) return "signal-zero"
        if (strength <= 25) return "signal-low"      // 1 bar
        if (strength <= 50) return "signal-medium"   // 2 bars
        if (strength <= 75) return "signal"          // 3 bars (good)
    ...
    function getSignalOpacity(strength) {
        ...
        if (strength === 0) return 0.3   // No signal
        if (strength <= 25) return 0.6   // Poor
        if (strength <= 50) return 0.8   // Fair
        if (strength <= 75) return 0.9   // Good
        return 1.0                        // Excellent
    }
    
    function getWifiIcon(isEnabled, strength, isConnected) {
        // 4-level granular WiFi signal display
        if (!isEnabled) return "wifi-off"
        if (!isConnected) return "wifi-off"
        
        if (strength === 0) return "wifi-zero"   // Connected but no signal
        if (strength <= 33) return "wifi-low"     // 1-2 bars (weak)
        if (strength <= 66) return "wifi"         // 2-3 bars (good) - use base icon
    ...
    function getWifiOpacity(isEnabled, strength, isConnected) {
        ...
        if (strength === 0) return 0.4   // Connected but no signal
        if (strength <= 33) return 0.6   // Weak
        if (strength <= 66) return 0.8   // Good
        return 1.0                        // Excellent
    }
