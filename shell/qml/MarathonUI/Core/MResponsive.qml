...
QtObject {
    ...
    function value(xsValue, smValue, mdValue, lgValue, xlValue, xxlValue) {
        if (isXXL && xxlValue !== undefined) return xxlValue
        if (isXL && xlValue !== undefined) return xlValue
        if (isLG && lgValue !== undefined) return lgValue
        if (isMD && mdValue !== undefined) return mdValue
        if (isSM && smValue !== undefined) return smValue
        return xsValue
    }
    
    function columns(defaultCols) {
        if (isXS) return Math.max(1, Math.floor(defaultCols / 4))
        if (isSM) return Math.max(1, Math.floor(defaultCols / 2))
        if (isMD) return Math.max(1, Math.floor(defaultCols * 0.75))
        return defaultCols
    }
}
