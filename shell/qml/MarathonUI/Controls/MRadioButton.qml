...
Item {
    ...
    Accessible.onPressAction: if (!disabled) select()
    
    Keys.onSpacePressed: if (!disabled) select()
    Keys.onReturnPressed: if (!disabled) select()
        ...
        if (!disabled && !checked) {
            ...
            if (groupName !== "" && parent) {
                for (var i = 0; i < parent.children.length; i++) {
                    ...
                    if (child !== root && 
                        child.hasOwnProperty("groupName") && 
    ...
    Row {
        ...
        Rectangle {
            ...
            border.color: {
                if (root.disabled) return MColors.textHint
                if (root.checked) return MColors.marathonTeal
