...
bool BluetoothAgent::registerAgent()
{
    ...
    if (!reply.isValid()) {
        qWarning() << "[BluetoothAgent] Failed to register with AgentManager:" << reply.error().message();
        bus.unregisterObject(m_agentPath);
        return false;
    }
    ...
    if (!defaultReply.isValid()) {
        qWarning() << "[BluetoothAgent] Failed to set as default agent:" << defaultReply.error().message();
...
void BluetoothAgent::DisplayPasskey(const QDBusObjectPath &device, quint32 passkey, quint16 entered)
{
    ...
    qInfo() << "[BluetoothAgent] Display passkey for device:" << deviceName 
