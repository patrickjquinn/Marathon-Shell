...
inline bool hasHardwareKeyboard() {
    ...
    if (!devicesFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qWarning() << "[Platform] Could not open /proc/bus/input/devices - assuming no hardware keyboard";
        return false;
    }
    ...
    if (!currentDeviceName.isEmpty() && hasKeyboardEV && currentHandlers.contains("kbd", Qt::CaseInsensitive)) {
        qInfo() << "[Platform] Hardware keyboard detected:" << currentDeviceName;
        return true;
    }
