...
{
    QString cacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) + "/marathon/app-store";
    QDir dir;
...
{
    QString cacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation) + "/marathon/app-store/downloads";
    QDir dir;
    if (!dir.exists(cacheDir)) {
        dir.mkpath(cacheDir);
    }
...
void MarathonAppStoreService::loadCachedCatalog()
{
    ...
    if (error.error != QJsonParseError::NoError) {
        qWarning() << "[MarathonAppStoreService] Failed to parse cached catalog:" << error.errorString();
        return;
    }
    ...
    if (doc.isArray()) {
        ...
        qDebug() << "[MarathonAppStoreService] Loaded cached catalog with" << m_catalog.size() << "apps";
    }
}
...
void MarathonAppStoreService::refreshCatalog()
{
    ...
    connect(m_catalogReply, &QNetworkReply::finished, this, &MarathonAppStoreService::handleCatalogReply);
}
...
void MarathonAppStoreService::handleCatalogReply()
{
    ...
    if (m_catalogReply->error() != QNetworkReply::NoError) {
        qWarning() << "[MarathonAppStoreService] Failed to fetch catalog:" << m_catalogReply->errorString();
...
QVariantList MarathonAppStoreService::searchApps(const QString &query)
{
    ...
    for (const QVariant &item : m_catalog) {
        ...
        if (name.contains(lowerQuery) || description.contains(lowerQuery) || id.contains(lowerQuery)) {
            results.append(app);
        }
    }
...
void MarathonAppStoreService::downloadApp(const QString &appId)
{
    ...
    connect(reply, &QNetworkReply::downloadProgress, this, &MarathonAppStoreService::handleDownloadProgress);
    connect(reply, &QNetworkReply::finished, this, &MarathonAppStoreService::handleDownloadFinished);
