...
void MarathonPermissionManager::loadPermissions()
{
    ...
    if (error.error != QJsonParseError::NoError) {
        qWarning() << "[MarathonPermissionManager] Failed to parse permissions:" << error.errorString();
        return;
    }
    ...
    qDebug() << "[MarathonPermissionManager] Loaded permissions for" << m_permissions.size() << "apps";
}
...
{
    qDebug() << "[MarathonPermissionManager] Setting permission:" << appId << permission << granted << remember;
...
MarathonPermissionManager::PermissionStatus MarathonPermissionManager::getPermissionStatus(const QString &appId, const QString &permission)
{
    if (!m_permissions.contains(appId)) {
        return NotRequested;
    }
