...
bool MarathonAppProcess::start(const QString &appPath, const QString &entryPoint)
{
    ...
    if (!m_process) {
        ...
        connect(m_process, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished),
                this, &MarathonAppProcess::handleFinished);
        connect(m_process, &QProcess::errorOccurred, this, &MarathonAppProcess::handleErrorOccurred);
...
void MarathonAppProcess::restart()
{
    ...
    QTimer::singleShot(1000, this, [this]() {
        start(m_appPath, m_entryPoint);
    });
}

void MarathonAppProcess::handleStarted()
{
    qInfo() << "[AppProcess]" << m_appId << "- Process started successfully (PID:" << m_process->processId() << ")";
...
void MarathonAppProcess::handleFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    qInfo() << "[AppProcess]   Exit status:" << (exitStatus == QProcess::NormalExit ? "Normal" : "Crashed");
    
    if (exitStatus == QProcess::CrashExit) {
        m_crashCount++;
        qCritical() << "[AppProcess]" << m_appId << "- CRASHED! (crash count:" << m_crashCount << "/" << MAX_CRASHES << ")";
