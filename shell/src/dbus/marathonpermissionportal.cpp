...
MarathonPermissionPortal::MarathonPermissionPortal(MarathonPermissionManager *permissionManager, QObject *parent)
    : QObject(parent)
    , m_permissionManager(permissionManager)
{
    // Connect permission manager signals to forward to D-Bus
    connect(m_permissionManager, &MarathonPermissionManager::permissionGranted,
            this, [this](const QString &appId, const QString &permission) {
    ...
    connect(m_permissionManager, &MarathonPermissionManager::permissionDenied,
            this, [this](const QString &appId, const QString &permission) {
        QString callerAppId = getCallerAppId();
        if (appId == callerAppId) {
...
bool MarathonPermissionPortal::CheckPermission(const QString &permission)
{
    ...
    qDebug() << "[MarathonPermissionPortal] Permission check:" 
             << appId << permission << hasPermission;
