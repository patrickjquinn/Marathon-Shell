...
bool MarathonNotificationService::registerService()
{
    ...
    if (!bus.registerService("org.marathon.NotificationService")) {
        qWarning() << "[NotificationService] Failed to register service:" << bus.lastError().message();
        return false;
    }
    ...
    if (!bus.registerObject("/org/marathon/NotificationService", this,
        ...
        qWarning() << "[NotificationService] Failed to register object:" << bus.lastError().message();
        return false;
    }
...
QVariantMap MarathonNotificationService::notificationToVariantMap(const NotificationDatabase::NotificationRecord &record)
{
    QVariantMap map;
    map["id"] = record.id;
