...
{
    connect(m_loader, &MarathonAppLoader::appLoaded, this, [this](const QString &appId) {
...
bool MarathonApplicationService::registerService()
{
    ...
    if (!bus.registerService("org.marathon.ApplicationService")) {
        qWarning() << "[ApplicationService] Failed to register service:" << bus.lastError().message();
        return false;
    }
    ...
    if (!bus.registerObject("/org/marathon/ApplicationService", this,
        ...
        qWarning() << "[ApplicationService] Failed to register object:" << bus.lastError().message();
        return false;
    }
...
QVariantMap MarathonApplicationService::GetAppInfo(const QString &appId)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (id == appId) {
            ...
            info["entryPoint"] = m_registry->data(index, MarathonAppRegistry::EntryPointRole).toString();
            break;
        }
    }
