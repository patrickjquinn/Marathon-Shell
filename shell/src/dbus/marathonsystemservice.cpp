...
MarathonSystemService::MarathonSystemService(PowerManagerCpp *power,
                                            NetworkManagerCpp *network,
                                            DisplayManagerCpp *display,
                                            AudioManagerCpp *audio,
                                            QObject *parent)
    : QObject(parent)
    , m_power(power)
    , m_network(network)
    ...
    , m_audio(audio)
{
    connect(m_power, &PowerManagerCpp::batteryLevelChanged, this, [this]() {
        emit BatteryChanged(batteryLevel(), batteryCharging());
    });
    
    connect(m_network, &NetworkManagerCpp::wifiConnectedChanged, this, [this]() {
        emit NetworkChanged(networkConnected(), networkType());
    });
    
    connect(m_network, &NetworkManagerCpp::ethernetConnectedChanged, this, [this]() {
        emit NetworkChanged(networkConnected(), networkType());
    });
}
...
{
    if (!m_network) return "none";
    if (m_network->wifiConnected()) return "wifi";
    if (m_network->ethernetConnected()) return "ethernet";
    return "none";
}
