...
QString MarathonAppVerifier::getTrustedKeysDir()
{
    ...
    QString userDir = QStandardPaths::writableLocation(QStandardPaths::ConfigLocation) + "/marathon/trusted-keys";
...
bool MarathonAppVerifier::initializeTrustedKeysDir()
{
    ...
    if (!dir.exists(keysDir)) {
        if (!dir.mkpath(keysDir)) {
            qWarning() << "[MarathonAppVerifier] Failed to create trusted keys directory:" << keysDir;
            return false;
        }
...
bool MarathonAppVerifier::verifySignatureFile(const QString &manifestPath, const QString &signaturePath)
{
    if (!isGPGAvailable()) {
    ...
    if (keyFiles.isEmpty()) {
        qDebug() << "[MarathonAppVerifier] No trusted keys found, accepting any valid signature (DEV MODE)";
    ...
    if (keyFiles.isEmpty() && gpgVerify.exitCode() == 0) {
        qDebug() << "[MarathonAppVerifier] No trusted keys configured, accepting valid signature (DEV MODE)";
        return true;
    }
...
MarathonAppVerifier::VerificationResult MarathonAppVerifier::verifyDirectory(const QString &appDir)
{
    ...
    if (!QFile::exists(signaturePath)) {
        ...
        qWarning() << "[MarathonAppVerifier] SIGNATURE.txt not found (development mode - allowing unsigned)";
...
MarathonAppVerifier::VerificationResult MarathonAppVerifier::verifyPackage(const QString &packagePath)
{
    ...
    m_lastError = "Direct package verification not yet implemented. Extract first, then verify directory.";
    emit verificationComplete(VerificationFailed);
    return VerificationFailed;
}

bool MarathonAppVerifier::signManifest(const QString &manifestPath, const QString &signaturePath, const QString &keyId)
{
