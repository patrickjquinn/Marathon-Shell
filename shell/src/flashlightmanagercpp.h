...
// Controls a flashlight/torch LED via sysfs (/sys/class/leds/*).
// Backend lives in C++ (perf + permissions + IO), QML should be a thin wrapper.
class FlashlightManagerCpp : public QObject {
  ...
  private:
    ...
    QString m_ledPath; // absolute path to LED dir in /sys/class/leds/<name>
