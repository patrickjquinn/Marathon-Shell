...
void NetworkManagerCpp::connectToNetwork(const QString& ssid, const QString& password)
{
    ...
    if (!m_hasNetworkManager || m_wifiDevicePath.isEmpty()) {
        qWarning() << "[NetworkManagerCpp] Cannot connect: NetworkManager or WiFi device not available";
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, ssid](QDBusPendingCallWatcher *watcher) {
...
void NetworkManagerCpp::setAirplaneMode(bool enabled)
{
    ...
    if (m_hasNetworkManager) {
        ...
        if (!m_wifiDevicePath.isEmpty()) {
            QDBusInterface wifiDevice("org.freedesktop.NetworkManager", 
                                     m_wifiDevicePath, 
                                     "org.freedesktop.DBus.Properties", 
                                     QDBusConnection::systemBus());
            wifiDevice.call("Set", "org.freedesktop.NetworkManager.Device", "Autoconnect", QVariant::fromValue(QDBusVariant(!enabled)));
        }
...
void NetworkManagerCpp::stopHotspot()
{
    ...
    QDBusInterface activeConn(
        "org.freedesktop.NetworkManager",
        m_hotspotConnectionPath,
        "org.freedesktop.NetworkManager.Connection.Active",
        QDBusConnection::systemBus()
    );
    
    if (activeConn.isValid()) {
        QDBusReply<void> reply = m_nmInterface->call("DeactivateConnection", 
                                                     QVariant::fromValue(QDBusObjectPath(m_hotspotConnectionPath)));
        if (reply.isValid()) {
...
void NetworkManagerCpp::connectVpn(const QString& connectionId)
{
    ...
    QDBusReply<QDBusObjectPath> reply = nmInterface.call("ActivateConnection", 
                                                          QVariant::fromValue(QDBusObjectPath(connectionId)),
...
void NetworkManagerCpp::disconnectVpn(const QString& connectionId)
{
    ...
    QDBusReply<QList<QDBusObjectPath>> activeConnsReply = nmInterface.call("Get",
                                                                            "org.freedesktop.NetworkManager",
                                                                            "ActiveConnections");
    ...
    for (const QDBusObjectPath &path : activeConnsReply.value()) {
        ...
        if (connPath == connectionId) {
            QDBusReply<void> reply = nmInterface.call("DeactivateConnection", 
