...
bool ShellIpcServer::registerOnSessionBus() {
    ...
    if (!bus.registerService("org.marathonos.Shell")) {
        // If another shell instance is running, this will fail; that's OK for dev,
        // but we should treat it as fatal for correctness.
        qWarning() << "[ShellIpcServer] Failed to register service org.marathonos.Shell:"
                   << bus.lastError().message();
        return false;
    }

    // Register dedicated DBus objects with strict caller enforcement.
