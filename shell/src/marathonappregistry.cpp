...
QVariant MarathonAppRegistry::data(const QModelIndex &index, int role) const
{
    ...
    switch (role) {
        ...
        case IconRole:
            return app->icon;
        case TypeRole:
            return static_cast<int>(app->type);
        case PathRole:
...
void MarathonAppRegistry::registerApp(const QString &id, const QString &name, const QString &icon,
                                       ...
                                       const QString &version, bool isProtected, const QStringList &permissions)
{
...
void MarathonAppRegistry::registerAppInfo(const AppInfo &info)
{
    ...
    qDebug() << "[MarathonAppRegistry] Registered app:" << info.id
             << "| Name:" << info.name
